import {useParams} from 'react-router-dom';
import ReactPlayer from 'react-player';
import './Trailer.css';

import React from 'react'

// const Trailer = () => {
//     let params =useParams();
//     let key=params.ytTrailerId;
//     console.log(key);
//   return (
//     <div className="react-player-container">
//       {(key!=null)?<ReactPlayer controls="true" playing={true} url={`https://www.youtube.com/watch?v=${key}`} 
//       width='100%' height='100%'/> : null}
//     </div>
//   )
// }
const Trailer = () => {
    let params = useParams();
    let key = params.ytTrailerId;
    
    try {
        if (key != null) {
            return (
                <div className="react-player-container">
                    <ReactPlayer controls="true" playing={true} url={`https://www.youtube.com/watch?v=${key}`} 
                    width='100%' height='100%'/>
                </div>
            );
        } else {
            return <div>No YouTube video ID provided.</div>;
        }
    } catch (error) {
        console.error('Error rendering trailer component:', error);
        return <div>An error occurred while rendering the trailer.</div>;
    }
};

export default Trailer
